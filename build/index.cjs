...
class KnexTransport extends winston_transport_1.default {
    /**
   ...
   */
    constructor(options = {}) {
        super(options);
        winston_transport_1.default.call(this, options);
    ...
    log(info, callback) {
        setImmediate(() => {
            ...
            const { client, tableName } = this;
            return client.transaction((trx) => trx(tableName)
    ...
    query(options, callback) {
        ...
        return client.transaction((trx) => trx.raw(sql.join(' ')))
            .then((res) => {
            callback(null, res);
        })
            .catch((err) => {
